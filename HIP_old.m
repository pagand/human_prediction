...
%     % Point estimate: average over all possibility
    nstep_pred_states_mean(k,:) = PXt1(k,:)*nstep_pred_states(:,:,k)+0.02*randn(1,4);
...
%     g_parts_satter_4plot(:,:,k) = G;
    ...
    if mod(k,5)==1
        ...
        hold on;scatter(nstep_pred_states_mean(1:i,1),nstep_pred_states_mean(1:i,2),50,'bx','LineWidth',2);
        %x particles
      ...
      %  quiver(G(:,1),G(:,2),delta_x_g,delta_y_g,0,'linewidth',2, 'MaxHeadSize',1.5)
        hold on; scatter(G(:,1),G(:,2),'o','cyan','filled');

        hold on;plot(Xshow(1:i,1),Xshow(1:i,2),'r','LineWidth',2);
        hold on;scatter(Xshow(1:i,1),Xshow(1:i,2),50,'ro','LineWidth',2);
        legend('Predicted trajectory','predicted position','Candidate goal','GT trajectory','GT position')
        hold on;
